...
# parabolas
n_parabolas = 8
coeffs = [random(-8,8) for _ in range(n_parabolas)]
max_hts = [random(-1,1) for _ in range(n_parabolas)]
n_lines = [int(random(10,30)) for _ in range(n_parabolas)]
do_shifts = [random(0,1)>0.2 for _ in range(n_parabolas)]

parabolas = []

for coeff, max_ht, n_line, do_shift in zip(coeffs, max_hts, n_lines, do_shifts):
